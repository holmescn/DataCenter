...
typedef struct tagRCV_DATA
{
	...
	union
	{
		RCV_REPORT_STRUCTEx m_pReport[1];		// 行情数据
		RCV_HISTORY_STRUCTEx m_pDay[1];			// 补日线数据
		RCV_MINUTE_STRUCTEx m_pMinute[1];		// 补分时数据
		RCV_POWER_STRUCTEx m_pPower[1];			// 补除权数据
		void* m_pData;							// 内存指针
	};
} RCV_DATA;
